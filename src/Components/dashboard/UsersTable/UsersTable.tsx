...
const UsersTable = () => {
  ...
  const [userDetails, setUserDetails] = useState<DataUsersType>(
    {} as DataUsersType
  );
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [isLoadingSearch, setIsLoadingSearch] = useState<boolean>(false);

  const handleGetAllUsers = (): Promise<DataUsersType[]> => {
  ...
  const handleSearchUsers = async (): Promise<void> => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}${searchByEmail}?emailPrefix=${searchQuery}`
      );
      ...
      <div className="w-full flex flex-col gap-4">
        ...
        <div className="hidden md:block overflow-x-auto rounded-lg shadow-md bg-white ">
          ...
          <table className="w-full  table-auto min-w-max text-center">
            ...
            <tbody>
              {dataUsers?.map((user: DataUsersType, index: number) => (
                <tr
                  key={user.id}
